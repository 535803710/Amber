...
输出契约为 v2，使用 `evidence` 代替旧 `timeline`。`detail` 默认为 `minimal`，默认最多返回 3 条，每条只包含用户需求、修改结果和涉及文件；`compact` 增加时间、分支及强关联提交，`full` 再增加来源和匹配依据。独立 Git 提交不会返回。需求、结果和文件列表均限制长度，输出不包含邮箱、会话 ID、Token、Webhook、附件、完整源码或完整 CLI 错误。
...
- `compact`：需要核对记录时间、分支或强关联 Git 提交时使用。
- `full`：需要排查来源、置信度、匹配原因或相关性分数时使用，不作为日常模式。
- `limit`：默认 3，范围 1～20；只有确有必要时才提高。
